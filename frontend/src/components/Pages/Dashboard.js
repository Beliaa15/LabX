...
const Dashboard = () => {
    ...
    const getPageTitle = () => {
        if (isAdmin()) return 'Admin Dashboard - LabX';
        if (isTeacher()) return 'Teacher Dashboard - LabX';
        if (isStudent()) return 'Student Dashboard - LabX';
        return 'Dashboard - LabX';
    };

    const getPageDescription = () => {
        if (isAdmin()) return 'Manage users, courses, and system settings on LabX platform.';
        if (isTeacher()) return 'Manage your courses, students, and assignments on LabX.';
        if (isStudent()) return 'Access your courses, assignments, and virtual labs on LabX.';
        return 'Your personalized dashboard on LabX virtual laboratory platform.';
    };
